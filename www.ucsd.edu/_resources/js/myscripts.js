...
function setSearch(searchType) {
    if (searchType == 'web') {
        ...
        $('input[data-id="inputSearchMobile"]').attr('id', 'entry').attr('name', 'entry').attr('placeholder', 'Search Faculty/Staff');
    }
}
...
$(window).on('resize', function () {
	...
	$('#heroslider').attr('style', 'margin-top:' + navHeight + 'px !important');
}).resize();
*/
        ...
        a.prepend(n),
            ...
            e.children("h2").click(function() {
                return (
                    ...
                    $(this).next().toggle(),
                    $(this).hasClass("expand") && (window.location.hash = $(this).find("a").text().replace(/\s/g, "-").substring(0, 31)), !1
                );
            }),
            a.find(".drawer-toggle a").click(function() {
                ...
                return (
                    ...
                    window.history && window.history.pushState ? window.history.pushState("", "", window.location.pathname) : (window.location.href = window.location.href.replace(/#.*$/, "#")), !1
                );
            }),
...
//Urgent: Temporary Patch for Search Results *******
document.addEventListener("DOMContentLoaded", () => {
    const cseScript = document.querySelector('script[src="https://cse.google.com/cse.js?cx=15cf43c1c5ee74fee"]');

    if (cseScript) {
        setInterval(() => {
            const divs = document.querySelectorAll('div.gs-result');

            divs.forEach(div => {
                const breadcrumbDiv = div.querySelector('div.gs-bidi-start-align.gs-visibleUrl.gs-visibleUrl-breadcrumb');
                if (breadcrumbDiv && breadcrumbDiv.innerHTML.includes('<span>act.ucsd.edu</span><span> › security</span><span> › financial</span><span> › login</span>')) {
                    div.remove();
                }
            });
        }, 1000); // 1 second
...
// Change Give button to Apply Now in nav and footer
...
(function() {
    ...
    if (month > 6 && month < 11) { // effective Aug-Nov
        // get/create footer elements
        const btnElem = document.querySelector('.btn-apply-footer');
        const aElem = btnElem.parentElement;
        ...
        // get/create nav elements
        const navAElem = document.querySelector('.btn-apply-nav');
        const spanElem2 = spanElem.cloneNode(true);

        // change footer elements
        btnElem.textContent = ' Apply Now';
        btnElem.prepend(spanElem);
        aElem.setAttribute('alt', 'apply now');
        aElem.setAttribute('href', 'https://admission.universityofcalifornia.edu/how-to-apply/apply-online/');
        h2Elem.remove();

        // change nav elements
        navAElem.textContent = ' Apply Now';
        navAElem.setAttribute('href', 'https://admission.universityofcalifornia.edu/how-to-apply/apply-online/');
        navAElem.prepend(spanElem2);
    }
})();